...
```{r}
tidy_model <- round_df(broom::tidy(lm(Encounters ~ ed_visit+government + investor+rural+ordered_LicensedBedSize, data = dat)), 4)
...
* **Coefficients**:
  * `ed_visit` has a positive and highly significant relationship with `Encounters`. For each unit increase in `ed_visit`, `Encounters` increases by about 5956.25, holding other variables constant.
  * `government` has a small negative effect, decreasing `Encounters` by 596.61 compared to the non-profit ownership category (likely private, non-government, non-investor).
  * `investor` ownership is associated with a larger negative effect, decreasing `Encounters` by 1307.89 compared to the non-profit
  * `rural` locations are associated with a negative effect, decreasing `Encounters` by 1503.58 compared to urban locations.
  * `ordered_LicensedBedSize` is treated as an ordinal variable with polynomial contrasts. The linear term (L) has a large positive effect, suggesting a strong increase in `Encounters` as bed size increases. However, the quadratic (Q), cubic (C), and quartic (^4) terms are not statistically significant at conventional levels, indicating the relationship may not have strong non-linear components.
* Model Fit: The Multiple R-squared value of 0.1275 suggests that approximately 12.75% of the variability in Encounters is explained by the model.